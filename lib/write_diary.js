...
exports.monthly_mode_A = function(req, res){
	...
	var focused_date = Number(req.query.date);
			//당해년도~3년전 일기까지 총 4년치 보여주기(물론 당해년도 일기는 우측 분할화면에서 작성중임...
		db.query(`SELECT * FROM monthly_diary WHERE id_number=? AND temperal_month=?`, [id_number, focused_month], function(err, monthly_diary){
			...
			for(answer = 0; answer < monthly_diary.length; answer++){
				if(monthly_diary[answer].temperal_year == focused_year){
					var monthly_diary_lv1 = monthly_diary[answer-3];
					var monthly_diary_lv2 = monthly_diary[answer-2];
					var monthly_diary_lv3 = monthly_diary[answer-1];
					var monthly_diary_lv4 = monthly_diary[answer-0];
				}
			}

			var content = `
			<div class="date" id="date_lv1">${monthly_diary_lv1.temperal_year}.${monthly_diary_lv1.temperal_month}</div>
			<div class="content" id="content_lv1">${monthly_diary_lv1.comment}</div>
			<div class="content_summary" id="content_summary_lv1">${monthly_diary_lv1.Question}</div>
			<div class="date" id="date_lv2">${monthly_diary_lv2.temperal_year}.${monthly_diary_lv2.temperal_month}</div>
			<div class="content" id="content_lv2">${monthly_diary_lv2.comment}</div>
			<div class="content_summary" id="content_summary_lv2">${monthly_diary_lv2.Question}</div>
			<div class="date" id="date_lv3">${monthly_diary_lv3.temperal_year}.${monthly_diary_lv3.temperal_month}</div>
			<div class="content" id="content_lv3">${monthly_diary_lv3.comment}</div>
			<div class="content_summary" id="content_summary_lv3">${monthly_diary_lv3.Question}</div>
			<div class="date" id="date_lv4">${monthly_diary_lv4.temperal_year}.${monthly_diary_lv4.temperal_month}</div>
			<div class="content" id="content_lv4">${monthly_diary_lv4.comment}</div>
			<div class="content_summary" id="content_summary_lv4">${monthly_diary_lv4.Question}</div>
			`;
