...
module.exports = {
	daily_template:function(title, content, uuid, focused_year, focused_month, focused_date, loaded_month){

		/*
		//이것은 [monthly]로 넘어가기 위해서만 사용한다. 쿼리스트링 데이터로 주려는것.
		var date_index_to_month_index = tools.switch_onlyM_from_D(date_index);
		//이것은 [오늘로]button 을 위해서만 사용
		var today_year_index = date_ob.getFullYear();
		var today_date_index = tools.switch_MD_to_D(date_ob.getMonth()+1, date_ob.getDate());
		...
		var today_month_index = tools.decode_d_index_M(horizontal_index);
		*/
