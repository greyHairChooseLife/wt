...
module.exports = {
	monthly_mode_B_template:function(title, content, calander, uuid, focused_year, focused_month, focused_date, loaded_month, daily_diary_length){
		...
		return `
			...
			var ii;
			for(ii=0; ii<${daily_diary_length}; ii++){
				function(magic_ii){
					document.getElementById("day"+(magic_ii+1)).addEventListener("click", function() {
						var i;
						for(i=0; i<${daily_diary_length}-1; i++){
							document.getElementById("day"+(i+2)).style.backgroundColor = "blue"; 
						}
						document.getElementById("day"+(magic_ii+1)).style.backgroundColor = "yellow"; 
						document.getElementById("content_"+magic_ii).style.display = "block";

						for(i=0; i<${daily_diary_length}-1; i++){
							document.getElementById("content_"+(i+1)).style.display = "none";
						}
					}, false);
				}(ii);
			}
